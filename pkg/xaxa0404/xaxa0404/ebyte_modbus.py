...
class subs_data(Node):
    ...
    def connect_slave(self):
        try:
            client = easymodbus.modbusClient.ModbusClient('/dev/ttyUSB1')
    ...
    def kirim_data(self):
        ...
        # write_int_register(self.modbus_client, 1, 0x0514, 4)
        ...
        # write_int_register(self.modbus_client, 1, 0x0064, 19000)
        # write_int_register(self.modbus_client, 1, 0x0065, 13400)
        # write_int_register(self.modbus_client, 1, 0x0066, 10230)
        # write_int_register(self.modbus_client, 1, 0x0067, 0)

        self.AI4 = read_input_register(self.modbus_client, 1, 0x0064,1)
        print(f"input 4 ={self.AI4}")

        # write_float_multi_register(self.modbus_client, 1, 0x0513, 0)
        # write_float_multi_register(self.modbus_client, 1, 0x0514, 0)
        # write_float_multi_register(self.modbus_client, 1, 0x0515, 0)
        # write_float_multi_register(self.modbus_client, 1, 0x0516, 0)
...
def read_input_register(modbus_client, slave_id, start_address, quantity):
    try:
        modbus_client.unitidentifier = slave_id
        registers = modbus_client.read_input_registers(start_address, quantity)
        return registers
    except Exception as e:
        print("Read Error:", e)
        return None    
