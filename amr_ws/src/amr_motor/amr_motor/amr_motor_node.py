...
Fix v5 (disesuaikan dengan zlac8015d.py v2 / V1_zlac_monitor):
  - _do_velocity_start: hapus clear_fault() manual karena write_velocity()
    sudah memanggil auto_clear_fault=True secara internal. Menghindari
    double clear_fault yang bisa membingungkan driver.
  - _do_position_start: hapus clear_fault() manual (sama, write_position
    sudah handle auto_clear_fault=True). Tambah start_delay_s=0.20 sesuai
    V1 monitor agar driver punya waktu proses sebelum START dikirim.
  - _do_torque_start: hapus clear_fault() manual (write_torque sudah handle).
  - _do_stop: hapus set_speed(0,0) sebelum stop() — cukup stop() saja agar
    tidak ada write ekstra yang bisa timing-conflict saat mode non-velocity.
  - _init_driver: set_accel_profile dipindah ke setelah set_velocity_mode()
    (urutan yang benar sesuai datasheet).
  - Semua pemanggilan write_velocity / write_position / write_torque tidak
    lagi memanggil clear_fault() secara manual sebelumnya.
"""
...
class AMRMotorNode(Node):
    ...
    def __init__(self):
        ...
        self.create_timer(1.0,   self.read_status_word_timer) # 1 Hz   – status word
        self.create_timer(1.0,   self.read_torque_timer)      # 1 Hz   – torque actual
    ...
    def _init_driver(self):
        ...
        try:
            # Fix v5: set_velocity_mode() dulu, BARU set_accel_profile()
            # (urutan yang benar sesuai datasheet dan V1 monitor)
    ...
    def _do_velocity_start(self, cmd: dict):
        try:
            ...
            # Fix v5: hapus clear_fault() manual — write_velocity() sudah
            # memanggil auto_clear_fault=True secara internal (default).
            # Double clear_fault bisa menyebabkan timing issue.
    ...
    def _do_position_start(self, cmd: dict):
        try:
            ...
            # Fix v5: hapus clear_fault() manual — write_position() sudah
            # memanggil auto_clear_fault=True secara internal (default).
            # Tambah start_delay_s=0.20 sesuai V1 monitor agar driver
            # punya waktu proses sebelum START command dikirim.
    ...
    def _do_torque_start(self, cmd: dict):
        try:
            ...
            # Fix v5: hapus clear_fault() manual — write_torque() sudah
            # memanggil auto_clear_fault=True secara internal (default).
    ...
    def _do_stop(self):
        try:
            # Fix v5: hapus set_speed(0,0) sebelum stop() — cukup stop() saja.
            # set_speed(0,0) hanya valid di velocity mode; di mode lain bisa
            # menyebabkan error. stop() (CONTROL_WORD=0x07) valid di semua mode.
            self.driver.stop()
            self._ui_controlled = False
            self.motor_enabled  = False
            self.target_rpm_l   = 0
    ...
    def _do_reset_data(self):
        ...
        self.pos_actual_l    = 0
        self.pos_actual_r    = 0
        self.torque_actual_l = 0.0
        self.torque_actual_r = 0.0
        self.motor_temp_l    = 0
        self.motor_temp_r    = 0
        self.driver_temp_raw = 0
        self.bus_voltage_raw = 0
        self.status_word     = 0
        self.error_code_l    = 0
        self.error_code_r    = 0
